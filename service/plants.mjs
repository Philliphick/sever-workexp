...
export const getAllPlants = async (params = null) => {
  if (params) {
    ...
    try {
      ...
      }; // Return the first matching plant or undefined if none found
    } catch (error) {
      console.error("Caught error:", error);
      throw new Error(
        "Sorry, no plant found with that name. Details: " + error.message
      );
    }
  } else {
    try {
...
//       bibliography: result.bibliography,
//       family: result.family,
//     }))[0]; // Return the first matching plant or undefined if none found
//   } catch (error) {
//     console.error("Caught error:", error);
//     throw new Error(
//       "Sorry, no plant found with that name. Details: " + error.message
